...
def build_model(num_classes=3, img_rows=8, img_cols=9, num_chan=4):
...
def rating_to_three_class(v, low=4.0, high=7.0):
    if v < low:
        return 0
    if v < high:
        return 1
...
def build_threeclass_labels_from_dat(dat_path, target="v", t=6, low=4.0, high=7.0):
    with open(dat_path, "rb") as f:
        obj = pickle.load(f, encoding="latin1")
    labels = np.asarray(obj["labels"], dtype=float)  # (40,4)
    col = 0 if target == "v" else 1
    trial_scores = labels[:, col]
    window_labels = np.repeat(trial_scores, 120)
    seq_labels = np.array(
        [rating_to_three_class(window_labels[j * t], low=low, high=high) for j in range(len(window_labels) // t)],
        dtype=int,
    )
    return to_categorical(seq_labels, 3)


def run_subject(mat_path, dat_path, target="v", epochs=50, batch_size=128, seed=7, low=4.0, high=7.0, save_models=False, model_dir=None, subject_id="00"):
    t = 6
    file = sio.loadmat(mat_path)
    data = file["data"]
    x = data.transpose([0, 2, 3, 1]).reshape((-1, t, 8, 9, 4))
    y = build_threeclass_labels_from_dat(dat_path, target=target, t=t, low=low, high=high)
    ...
    for fold_idx, (train, test) in enumerate(kfold.split(x, y.argmax(1)), start=1):
        ...
        with strategy.scope():
            model1, model2, model3 = build_model(num_classes=3)
        ...
        callbacks1, callbacks2, callbacks3 = [], [], []
        if save_models and model_dir is not None:
            fold_dir = os.path.join(model_dir, f"subject_{subject_id}", f"fold_{fold_idx}")
            os.makedirs(fold_dir, exist_ok=True)
            callbacks1.append(ModelCheckpoint(os.path.join(fold_dir, "best_model1.keras"), monitor="val_accuracy", mode="max", save_best_only=True, save_weights_only=False, verbose=0))
            callbacks2.append(ModelCheckpoint(os.path.join(fold_dir, "best_model2.keras"), monitor="val_accuracy", mode="max", save_best_only=True, save_weights_only=False, verbose=0))
            callbacks3.append(ModelCheckpoint(os.path.join(fold_dir, "best_model3.keras"), monitor="val_accuracy", mode="max", save_best_only=True, save_weights_only=False, verbose=0))
...
def main():
    ...
    ap.add_argument("--dat-dir", default="/root/inpainting-work/interactive/new/data_preprocessed_custom")
    ap.add_argument("--target", choices=["v", "a"], default="a")
    ap.add_argument("--subjects", default="all")
    ap.add_argument("--epochs", type=int, default=50)
    ap.add_argument("--batch-size", type=int, default=128)
    ap.add_argument("--low", type=float, default=4.0, help="Three-class lower threshold.")
    ap.add_argument("--high", type=float, default=7.0, help="Three-class upper threshold.")
    ap.add_argument("--save-models", action="store_true")
    ap.add_argument("--model-dir", default="/root/inpainting-work/interactive/TDMNN/models_threeclass")
    ap.add_argument("--out-json", default="/root/inpainting-work/interactive/TDMNN/deap_threeclass_results.json")
    args = ap.parse_args()
    ...
    all_means, all_stds, details = [], [], {}
    for sid in subs:
        mat_path = os.path.join(args.dataset_dir, f"DE_s{sid}.mat")
        dat_path = os.path.join(args.dat_dir, f"s{sid}.dat")
        if not (os.path.exists(mat_path) and os.path.exists(dat_path)):
    ...
    out = {
        "task": "valence_3class" if args.target == "v" else "arousal_3class",
